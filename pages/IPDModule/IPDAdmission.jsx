...
export const IPDAdmission = (props) => {
  ...
  const getAdmissionResources = async () => {
    ...
    try {
      const response = await axios.post(
        "http://192.168.1.108:5000/admission_resources"
      );
  ...
  const getFilteredPatients = async (input) => {
    try {
      const response = await axios.post(
        "http://192.168.1.108:5000/filtered_patient",
        {
          like_name: input,
        }
      );
  ...
  const updateIPDAdmission = async (input) => {
    try {
      const response = await axios.post(
        "http://192.168.1.108:5000/update-ipd-details",input
      );
  ...
  const handleMRDClick = (IPDNo) => {
    const url = `/MRD-form?IPDNo=${IPDNo}`;
    window.open(url, "_blank"); // Opens in a new tab
  };

  const handleAdmissionClick = (IPDNo) => {
    const url = `/Admission-form?IPDNo=${IPDNo}`;
    window.open(url, "_blank"); // Opens in a new tab
  };
    ...
    <>
      <Box paddingX={5} paddingY={5}>
        <Grid container marginY={1} display="flex"  width="100%">
          ...
          <Grid item xs={1} paddingX={1} alignItems="center" >
            <Typography fontSize="13px" fontWeight="bold">
              Date:
            </Typography>
            <TextField
        ...
        <Grid container marginY={2} display="flex"  width="100%">
          ...
          >
            ...
            <TextField value={bedNo} size="small" disabled/>
