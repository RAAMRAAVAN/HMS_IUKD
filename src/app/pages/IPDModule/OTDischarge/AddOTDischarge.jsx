...
export const AddOTDischarge = (props) => {
  ...
  const getOTDischargeFormats = async () => {
    try {
      let result = await axios.get("http://192.168.1.108:5000/getOTDischargeFormats");
  ...
  const updateOTDischargeDate = async (printStatus) => {
    try {
      let status = await axios.post("http://192.168.1.108:5000/updateOTDischargeDate", { Date: date, Time: time, AID: AID });
      alert("Date Updated");
      if (printStatus === true)
        handlePrintClick();
    } catch (err) {
